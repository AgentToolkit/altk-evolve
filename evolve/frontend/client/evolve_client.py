...
from evolve.llm.fact_extraction.fact_extraction import ExtractedFact, extract_facts_from_messages
from evolve.schema.conflict_resolution import EntityUpdate
from evolve.schema.core import Entity, Namespace, RecordedEntity
from evolve.schema.exceptions import NamespaceAlreadyExistsException, NamespaceNotFoundException
from evolve.schema.tips import ConsolidationResult
...
class EvolveClient:
    ...
    def __init__(self, config: EvolveConfig | None = None):
        ...
        if self.config.backend == "milvus":
            from evolve.backend.milvus import MilvusEntityBackend

            self.backend = MilvusEntityBackend(self.config.settings)
        elif self.config.backend == "filesystem":
            from evolve.backend.filesystem import FilesystemEntityBackend, FilesystemSettings
    ...
    def cluster_tips(self, namespace_id: str, threshold: float | None = None, limit: int = 10000) -> list[list[RecordedEntity]]:
        ...
        """
        from evolve.llm.tips.clustering import cluster_entities
