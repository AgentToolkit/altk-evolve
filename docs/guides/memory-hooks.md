...
- **Native vs raw CPEX.** Native plugins (normalizer, access stamp, READI) import no cpex and run through the adapter. `PIIFilterMemoryPlugin` is the one **raw CPEX** plugin: it subclasses `cpex-pii-filter`'s `Plugin` to alias it onto Evolve's hook types, so it is registered directly (no adapter), and needs the `[pii-regex]` extra (cpex + cpex-pii-filter; `[pii]` is a back-compat alias). `ReadiSemanticPIIPlugin` is native and needs the `[pii-semantic]` extra for its detector.
