...
A schedule PUT body contains `definition` and `expected_revision` (zero for creation). See [retention scheduling](retention-scheduling.md) for the definition, worker setup, timing semantics, and recovery. Mounting the API does not launch the worker. The host can use friendly UI controls to generate cron and display the preview without asking end users to read cron expressions.
