#!/usr/bin/env python3
"""Benchmark: how effectively does each PII redactor actually remove PII? (issue #275)

This is the evidence behind shipping a *semantic* redaction plugin alongside the
regex one. It scores a redactor against a labeled gold set — text with known PII
spans — and reports the numbers that matter for compliance:

  * recall      — fraction of true PII spans that got masked (the "did we remove
                  it?" number; 1 - recall is the span-level leak rate)
...
Both backends are scored through the same interface: the ``SpanDetector``
callable from ``altk_evolve.hooks.plugins.readi`` (text -> character spans), so
the harness is engine-agnostic and the scoring is apples-to-apples.

The default gold set is generated deterministically from templates with known
values, so offsets are exact and the run needs no network or external corpus.

Against a real corpus instead:
...
# regex backend supports; `person` and `address` are deliberately included to
# expose the no-NER gap honestly.
...
# Canonical entity types the regex backend actually targets. Used to report a
# "supported-subset" recall: of the PII types it claims to detect, how well does
# it remove them? (Distinct from overall recall, which is dragged down by types
# it has no detector for — names, addresses, DOB, IBAN, ...)
...
# labels are lowercased as-is (firstname, street, dob, ...) so per-label recall
# still shows them (at recall 0 for the regex backend).
AI4_NORMALIZE = {
    "EMAIL": "email",
    "PHONENUMBER": "phone",
...
# ── detectors (both expressed as altk_evolve SpanDetector callables) ──


def byte_spans_to_char_spans(text: str, spans: Iterable[tuple[int, int]]) -> list[tuple[int, int]]:
    """Convert UTF-8 **byte** offsets into Python **character** offsets.

    cpex-pii-filter is a Rust regex engine and reports byte offsets. Scoring
    them as if they were character offsets silently mis-scores every multibyte
    script: on Japanese it dragged measured precision from 0.99 down to 0.31,
    because every span landed in the wrong place. For pure-ASCII text this is
    the identity function. Pure, so it is unit-tested in
    ``tests/unit/test_readi_redaction_core.py``.
    """
    data = text.encode("utf-8")
    return [(len(data[:start].decode("utf-8", "ignore")), len(data[:end].decode("utf-8", "ignore"))) for start, end in spans]
...
def build_regex_detector(entities: list[str] | None = None, custom_patterns: list[dict] | None = None):
    ...
    if custom_patterns:
        options["custom_patterns"] = custom_patterns
    detector = PIIDetectorRust(options)

    def detect(text: str) -> list[tuple[int, int]]:
        if not text:
            return []
        raw = detector.detect(text)
        byte_spans = [(s["start"], s["end"]) for spans in raw.values() for s in spans]
        return byte_spans_to_char_spans(text, byte_spans)
...
def load_hf(dataset_id: str, split: str, limit: int, language: str | None) -> list[dict]:
    ...
    normalized to our canonical types where the regex backend has an equivalent.
    """
...
def load_wikiann(dataset_id: str, config: str, split: str, limit: int) -> list[dict]:
    ...
    BIO tags folded into PER/ORG/LOC character spans. Used to benchmark NER
    coverage on languages ai4privacy doesn't cover.
    """
...
def score(records: list[dict], detect) -> dict:
    """Score a SpanDetector against gold records. Redaction uses the shipped core."""
    from altk_evolve.hooks.plugins.readi import redact_spans
...
def _print(title: str, result: dict) -> None:
    ...
    print(f"  recall on regex-supported types only={result['supported_recall']:.2f}  (over {result['supported_total']} spans)")
...
def main() -> int:
    ...
    parser.add_argument("--dataset", help="HF dataset id of an ai4privacy-style set, e.g. ai4privacy/pii-masking-200k.")
    parser.add_argument("--split", default="train", help="HF split to stream (default: train).")
    parser.add_argument("--limit", type=int, default=1000, help="Max records to score (default: 1000).")
    ...
    parser.add_argument(
        ...
        default="regex",
        help="Backend(s) to benchmark: regex (cpex-pii-filter), semantic (IBM READI NER), or both.",
    )
    ...
    # Semantic-backend selection, mirroring the plugin's YAML config keys.
    ...
    if want_regex:
        _print("cpex regex — structured entities only", score(records, build_regex_detector()))
        if not args.dataset:
            ...
            _print("cpex regex + custom name patterns", score(records, build_regex_detector(custom_patterns=name_patterns)))

    if want_semantic:
        from altk_evolve.hooks.plugins.readi import build_readi_detector
        ...
        detector = build_readi_detector(
            extractor=args.readi_extractor,
            model=args.readi_model,
            language=args.readi_language,
        )
        _print(label, score(records, detector))

    print("\nNotes:")
    print("  - 'recall on regex-supported types only' is the fair number for the regex backend.")
    print("  - regex excels at structured PII at precision ~1.0 but has no NER, so it misses")
    print("    names/addresses; semantic (READI) catches those free-form entities.")
    print("  - for non-English, pass a language-matched --readi-model (e.g. ja_core_news_trf).")
    return 0
