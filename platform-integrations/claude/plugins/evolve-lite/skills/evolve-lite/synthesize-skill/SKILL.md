...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/evolve-lite/synthesize-skill/scripts/synthesize.py \
  finalize \
...
- Copy the directory into `.evolve/skills/<name>/` (canonical), then mirror it into `.claude/skills/<name>/` so Claude Code's skill loader sees it.
...
If the validator rejects the draft, fix the SKILL.md and retry — do not edit files under `.evolve/skills/` or `.claude/skills/` directly.
