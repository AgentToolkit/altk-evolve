#!/usr/bin/env python3
"""
Evolve Skill: Recall (Stage 2 Filesystem Backend)
Reads entities from .evolve/entities.json and outputs them in a compact format.
...
def main():
    parser = argparse.ArgumentParser(description="Get Evolve entities (Stage 2 Filesystem)")
    ...
    entities_file = workspace_root / ".evolve" / "entities.json"

    if not entities_file.exists():
        print("No Evolve guidelines exist yet. Complete some tasks to generate learnings!")
        sys.exit(0)
    ...
    print(f"## EVOLVE {args.type.upper()}S ({len(results)} found)\n")
