...
```bash
python3 <path-to-evolve-recall-dir>/scripts/get.py --type guideline --task "<brief summary>"
```
...
7. **DO generate entities like these** (specific, learned):
   ...
   - "The evolve save.py script reads from stdin only; do not pass CLI arguments" — project-specific, error-derived
...
```bash
printf '{"entities": [...]}' | python3 <path-to-evolve-learn-dir>/scripts/save.py
```
*(Use `python` if `python3` isn't found)*
...
python3 <path-to-evolve-learn-dir>/scripts/save.py --task "..." --outcome "..."
...
printf '{"entities": [{"content": "Use \"quotes\" here"}]}' | python3 <path-to-evolve-learn-dir>/scripts/save.py
