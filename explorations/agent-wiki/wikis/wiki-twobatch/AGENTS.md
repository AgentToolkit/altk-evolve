...
guideline/cluster/task/subtask page. The schema:

```json
{
  "kind": "guideline" | "cluster" | "task" | "subtask",
  "id": "<12-hex-char content hash, OR cluster:<slug>, OR task:<slug>, OR subtask:<slug>>",
  "title": "<short title>",
  "tags": ["...", "..."],
...
Rows are sorted **clusters first** so a naive top-N reader prefers them.
Cluster pages are *aggregators* — when a cluster matches your query, it
references its member atomic guidelines; you usually don't need to read
the members directly unless you want the original wording or its source
trajectory.
