...
def sync_phoenix(
    ...
    """
    Import trajectories from Arize Phoenix into the local system and generate tips.
    
    Prints the sync configuration, runs the Phoenix sync process, displays a results table
    with metrics (trajectories processed, trajectories skipped, tips generated, and errors),
    and prints error details when present. On unexpected failure the command exits with
    code 1.
    
    Parameters:
        phoenix_url (Optional[str]): Phoenix server URL to connect to. If None, the syncer
            will use its default or configured URL.
        namespace (Optional[str]): Target Kaizen namespace ID to store generated tips.
        project (Optional[str]): Phoenix project name to sync.
        limit (int): Maximum number of spans/trajectories to fetch from Phoenix.
        include_errors (bool): If True, include failed/error spans in the sync.
    """
