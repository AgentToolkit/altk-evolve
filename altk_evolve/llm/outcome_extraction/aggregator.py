...
where `value` is 1.0 / 0.0 for success / failure (UNKNOWN observations are
excluded from both numerator and denominator). Output range [0, 1] reads as
probability-of-success: 0.0 = all failures, 0.5 = no data / neutral,
1.0 = all successes.
...
`compute_cold_start_prior` directly — the prior is already in [0, 1].
"""
...
def aggregate(
    ...
    *,
    ...
    for obs in observations:
        ...
        # value is 1.0 (success) or 0.0 (failure) so the score lands in [0, 1].
    ...
    if measured == 0:
        # Cold start: prior is already in [0, 1] (probability of success).
        prior = compute_cold_start_prior(category=category, source=source)
        out.confidence_weighted_score = max(0.0, min(1.0, prior))
    else:
        out.confidence_weighted_score = weighted_sum / weight_sum if weight_sum > 0 else 0.5
